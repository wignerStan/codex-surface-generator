...
This extractor owns server response placement and normalization semantics that are
not request/client-metadata facts: outer HTTP response headers, WebSocket upgrade
response headers, event-embedded header containers, effective-model routing
feedback, and the X-Models-Etag -> /models catalog refresh chain.
"""
...
class ResponsesServerResponseExtractor:
    ...
    def extract(self, snapshot: SourceSnapshot, diagnostics: DiagnosticCollector) -> ExtractorResult:
        sources = {spec_id: snapshot.files.get(spec_id) for spec_id in SOURCE_SPECS}
        if any(source is None for source in sources.values()):
            for spec_id, source in sources.items():
                if source is None:
                    _missing(
                        diagnostics,
        ...
        ws = sources[WS]
        turn = sources[TURN]
        models_manager = sources[MODELS_MANAGER]
        models_endpoint = sources[MODELS_ENDPOINT]
        model_provider_models = sources[MODEL_PROVIDER_MODELS]
        assert common and sse and ws and turn and models_manager and models_endpoint and model_provider_models

        complete = _require(
            diagnostics,
            source=common,
            entity="responses_server_response.normalized_event",
            tokens=(
        ...
        complete &= _require(
            diagnostics,
            source=sse,
            entity="responses_server_response.http_sse",
            tokens=(
                ...
                ("RESPONSES_EVENT_OPENAI_MODEL_ALIAS_MISSING", 'eq_ignore_ascii_case("x-openai-model")'),
        ...
        complete &= _require(
            diagnostics,
            source=ws,
            entity="responses_server_response.websocket",
            tokens=(
        ...
        complete &= _require(
            diagnostics,
            source=turn,
            entity="responses_server_response.models_catalog.dispatch",
            tokens=(
        ...
        complete &= _require(
            ...
            entity="responses_server_response.models_catalog.provider_bridge",
            tokens=(
        ...
        complete &= _require(
            ...
            entity="responses_server_response.models_catalog.http",
            tokens=(
        ...
        response_model_body = _brace_body(sse.text, "pub fn response_model(&self) -> Option<String>")
        response_model_uses_response_model_field = bool(
            response_model_body
            and ('response.get("model")' in response_model_body or 'response["model"]' in response_model_body)
        )
        if response_model_body is None:
            complete = False
            _missing(
                diagnostics,
        ...
        elif response_model_uses_response_model_field:
            complete = False
            diagnostics.emit(
        ...
        manager_refresh_body = _brace_body(models_manager.text, "async fn refresh_if_new_etag")
        if manager_refresh_body is None:
            complete = False
            _missing(
                diagnostics,
        ...
        body = {
            ...
            "effective_model": {
                ...
                "response_model_payload_field_consulted": response_model_uses_response_model_field,
                "not_client_metadata": True,
                "not_turn_metadata": True,
            },
            "models_catalog_invalidation": {
                "signal": {
                    ...
                    "http_sse": {
                        "location": "outer HTTP response header",
            ...
            "evidence": {
                "common": _evidence(common, "ResponseEvent::ServerModel/ModelsEtag"),
                "sse": _evidence(sse, "spawn_response_stream/ResponsesStreamEvent::response_model"),
                "websocket": _evidence(ws, "connect_websocket/run_websocket_response_stream"),
                "turn_dispatch": _evidence(turn, "ResponseEvent::ModelsEtag -> refresh_if_new_etag"),
                "models_manager": _evidence(models_manager, "refresh_if_new_etag/fetch_and_update_models"),
                "models_endpoint": _evidence(models_endpoint, "ModelsClient::request_url/list_models"),
                "model_provider_models": _evidence(model_provider_models, "OpenAiModelsEndpoint::list_models"),
            },
        }
        return _result(body, complete)
